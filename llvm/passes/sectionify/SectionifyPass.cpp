...
#if LLVM_VERSION >= 37
    for (auto &i : sectionMapOpt)
        functionSectionMapOpt.push_back(i);
    for (auto &i : sectionMapOpt)
        dataSectionMapOpt.push_back(i);
#else
...
#if LLVM_VERSION >= 37
      GlobalObject *GO = dyn_cast<GlobalObject>(value);
      assert(GO);
#endif
