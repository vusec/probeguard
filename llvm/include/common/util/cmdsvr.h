...
static void *util_cmdsvr_func(void *conf_arg)
{
    ...
    _UTIL_PRINTF("fd:%d sun_family=%d, sun_path=%s, size: %lu\n", 
		conf->listen_fd, conf->local_sa.sun_family, conf->local_sa.sun_path, 
		local_sz);
    ret = bind(conf->listen_fd, (struct sockaddr*)&conf->local_sa,local_sz);
    if (ret == -1) {
	_UTIL_PRINTF("cmdsvr: bind failed. Errno: %s\n", strerror(errno));
    }
    ...
    do {
        ...
        if (req->size > req->max_size || req->size < req->min_size) {
            _UTIL_PRINTF("cmdsvr: Received incorrect sized request (expected [%d;%d], received %d)\n",
